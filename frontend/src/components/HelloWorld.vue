      ...
      <div class="container h-100">
        <div class="row justify-content-center h-100">
          <div class="col-12 form" v-if="!ws">
            ...
            <div class="input-group">
              <input v-model="user.username" class="form-control username" placeholder="username"/>

              <input v-model="user.password" type="password" class="form-control password" placeholder="password"/>
              <input v-model="user.confirm_password" type="password" class="form-control password" placeholder="confirm password"/>
  ...
  methods: {
    ...
    async register() {
      try {
        ...
        const result = await this.axios.post("http://localhost:8080/api/v1/auth/register", this.user);
        if (result.data.status !== "undefined" && result.data.status == "error") {
          this.loginError = "Register failed 1";
        } else {
          this.user.token = result.data;
          this.connectToWebsocket();
        }
      } catch (e) {
        this.loginError = "Register failed 2";
        console.log(e);
      }
    },
    async login() {
      try {
        //const result = await this.axios.post("http://" + location.host + '/api/v1/auth/login', this.user);
        const result = await this.axios.post("http://localhost:8080/api/v1/auth/login", this.user);
        if (result.data.status !== "undefined" && result.data.status == "error") {
